...
fn test_sudoku_solution1() {
    ...
    if let Some(solution) = solutions.first() {
        let solved_grid = decode_solution(solution, &mapping);
        println!("\nSolved puzzle:");
        print_sudoku(&solved_grid);
        assert!(is_valid_sudoku(&solved_grid));
    } else {
        panic!("No solution found!");
    }
}
#[test]
fn test_sudoku_solution2() {
    let puzzle = [
        [2, 9, 5, 7, 4, 3, 8, 6, 1],
        [4, 3, 1, 8, 6, 5, 9, 0, 0],
        [8, 7, 6, 1, 9, 2, 5, 4, 3],
